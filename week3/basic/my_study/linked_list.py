"""
코드 참고: https://wikidocs.net/225030
포인터 방식이 상당히 헷갈린다. 하루 종일 Linked List 구현만 붙잡고 있었다.
"""
...
class LinkedList:
    ...
    def remove(self, target):
        ...
        while node is not None and node.data != target:
            prev = node
            node = node.next
        if node is None:  # NOTE: 값을 못 찾았거나 head가 none
            return False
    ...
    def insert(self, i, data):
        if i == 0:
            ...
            if i > 0:
                num_of_repetitions = i - 1
            elif i >= -self.length:
                num_of_repetitions = (
                    self.length + i
                )  # NOTE: i는 음수 (내가 추가한 기능)
            else:
                self.appendleft(data)
    ...
    def reverse(self):
        if self.length < 2:
            return
        # NOTE: self.head를 현재 노드로 사용하는데 너무 가독성이 안 좋아서 AI 도움을 받아 수정
        # prev = None
