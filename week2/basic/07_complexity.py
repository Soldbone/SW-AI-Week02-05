...
def find_duplicates_brute_force(nums):
    ...
    for one_idx in range(n - 1):
        for other_idx in range(one_idx + 1, n):
            if (nums[one_idx] == nums[other_idx]) and (nums[one_idx] not in duplicates):
                duplicates.append(nums[one_idx])
